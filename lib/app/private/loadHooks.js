  ...
  return function initializeHooks(hooks, cb) {
    ...
    function prepareHook(id) {
      ...
      if (!_.isFunction(hookPrototype)) {
        ship.log.error('Malformed hook! (' + id + ')');
        ship.log.error('Hooks should be a function with one argument (`ship`)');
        process.exit(1);
      }

      // Instantiate the hook
      var def = hookPrototype(ship);
    ...
    function applyDefaults(hook) {
      ...
      defaultsDeep(ship.config, defaults);
    }

    // Load a hook (bind its routes, load any modules and initialize it)
    function loadHook(id, cb) {

      var timeoutInterval = (ship.config[hooks[id].configKey || id] && ship.config[hooks[id].configKey || id]._hookTimeout) || ship.config.hookTimeout || 20000;
      var hookTimeout;
      if (id != 'userhooks') {
        hookTimeout = setTimeout(function tooLong() {
          var hooksTookTooLongErr = 'The hook `'+id+'` is taking too long to load.\n' +
            ...
            'or else set `ship.config.' + (hooks[id].configKey || id) +
      ...
      hooks[id].load(function(err) {
        ...
        if (err) {
          if (id != 'userhooks') {
            console.log('A hook (`' + id + '`) failed to load!');
          }
          //ship.emit('hook:' + id + ':error');
          return cb(err);
        }

        //ship.log.verbose(id, 'hook loaded successfully.');
        //ship.emit('hook:' + id + ':loaded');
    ...
    async.series({
        ...
        load: function(cb) {
          async.each(_.without(_.keys(hooks), 'userconfig', 'moduleloader', 'userhooks'), function (id, cb) {
            //ship.log.silly('Loading hook: ' + id);
            loadHook(id, cb);
          }, cb);
        }
      },
